...
const PAGENTIC_DEFAULT_PROXY_URL = "https://llm.patternautomation.io/default/"
const PAGENTIC_FAST_LLM_PROXY_URL = "https://llm.patternautomation.io/fast/"
const PAGENTIC_SMART_LLM_PROXY_URL = "https://llm.patternautomation.io/smart/"
const DEFAULT_PAGENTIC_MODEL_FAMILY_URL = "https://llm.patternautomation.io/api/model_family"
const DEFAULT_PAGENTIC_MODEL = "openai"  // Fallback model family
...
// PAgentic API response structure
interface PAgenticModelConfig {
...
export class LangChainProvider {
  ...
  // Model config cache for PAgentic - now stores full API response
  private modelConfigCache: { config: PAgenticModelConfig; timestamp: number } | null = null
  ...
  async getModelCapabilities(): Promise<ModelCapabilities> {
    ...
    if (provider.modelConfig?.contextWindow) {
      ...
      switch (provider.type) {
        case 'pagentic':
          // PAgentic Browser uses gemini 2.5 flash by default
          maxTokens = 1_000_000
          break
  ...
  private _getDefaultModelForProvider(type: string, intelligence: string  = 'high'): string {
    switch (type) {
      case 'pagentic':
      case 'browseros':  // Backward compatibility
  ...
  private async _createLLMFromProvider(
    ...
    switch (provider.type) {
      case 'pagentic':
      case 'browseros':  // Backward compatibility
        // Only PAgentic Browser uses intelligence parameter
        const intelligence = options?.intelligence ?? DEFAULT_INTELLIGENCE
        return await this._createPAgenticLLM(temperature, maxTokens, streaming, intelligence)
      ...
      default:
        Logging.log('LangChainProvider',
          `Unknown provider type: ${provider.type}, falling back to PAgentic`,
          'warning')
        const defaultIntelligence = options?.intelligence ?? DEFAULT_INTELLIGENCE
        return await this._createPAgenticLLM(temperature, maxTokens, streaming, defaultIntelligence)
    }
  }
  
  // Fetch model configuration from PAgentic API
  private async _fetchModelConfig(): Promise<PAgenticModelConfig> {
    if (this.modelConfigCache) {
    ...
    try {
      const response = await fetch(DEFAULT_PAGENTIC_MODEL_FAMILY_URL)
      if (response.ok) {
        const data = await response.json()

        // Ensure all fields are present with fallbacks
        const config: PAgenticModelConfig = {
          default: data.default || data.model_family || DEFAULT_PAGENTIC_MODEL,
          fast: data.fast || data.model_family || DEFAULT_PAGENTIC_MODEL,
          smart: data.smart || data.model_family || DEFAULT_PAGENTIC_MODEL
        }

        // Cache the result
        this.modelConfigCache = { config, timestamp: Date.now() }
        Logging.log('LangChainProvider',
          `PAgentic model config fetched - default: ${config.default}, fast: ${config.fast}, smart: ${config.smart}`,
          'info')
        return config
      }
    } catch (error) {
    ...
    const fallbackConfig: PAgenticModelConfig = {
      default: DEFAULT_PAGENTIC_MODEL,
      fast: DEFAULT_PAGENTIC_MODEL,
      smart: DEFAULT_PAGENTIC_MODEL
    }
    return fallbackConfig
  }

  // PAgentic built-in provider (uses proxy, no API key needed)
  private async _createPAgenticLLM(
    temperature: number,
    maxTokens?: number,
