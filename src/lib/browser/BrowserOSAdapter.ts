...
export class BrowserOSAdapter {
  ...
  async getInteractiveSnapshot(
    ...
    try {
      ...
      if (
        ...
        chrome.browserOS &&
        'getInteractiveSnapshot' in chrome.browserOS &&
        typeof chrome.browserOS.getInteractiveSnapshot === 'function'
      ) {
        ...
        // Fallback for regular Chrome browsers without browserOS API
        console.warn('[BrowserOSAdapter] chrome.browserOS.getInteractiveSnapshot not available, using fallback');
        return this._getFallbackInteractiveSnapshot(tabId, options);
      }
    } catch (error) {
      const errorMessage =
  ...
  /**
   * Fallback implementation for getInteractiveSnapshot when chrome.browserOS is not available
   */
  private async _getFallbackInteractiveSnapshot(
    tabId: number,
    options?: InteractiveSnapshotOptions,
  ): Promise<InteractiveSnapshot> {
    try {
      // Use content script to extract interactive elements
      ...
      // Convert to InteractiveSnapshot format
      const snapshot: InteractiveSnapshot = {
        snapshotId: Date.now(),
        timestamp: Date.now(),
        elements: results.elements || [],
        hierarchicalStructure: results.hierarchicalStructure || '',
        processingTimeMs: results.processingTime || 0
      };

      console.log(
        `[BrowserOSAdapter] Fallback snapshot retrieved with ${snapshot.elements.length} elements`,
      );

      return snapshot;
    } catch (error) {
      // If content script fails, return minimal snapshot
      console.warn('[BrowserOSAdapter] Fallback snapshot failed, returning minimal snapshot:', error);
      return {
        snapshotId: Date.now(),
        timestamp: Date.now(),
        elements: [],
        hierarchicalStructure: 'Snapshot unavailable',
        processingTimeMs: 0
      };
    }
  }
  ...
  async captureScreenshot(
    ...
    try {
      ...
      if (
        typeof chrome !== 'undefined' &&
        chrome.browserOS &&
        'captureScreenshot' in chrome.browserOS &&
        typeof chrome.browserOS.captureScreenshot === 'function'
      ) {
        ...
        // Fallback for regular Chrome browsers without browserOS API
        console.warn('[BrowserOSAdapter] chrome.browserOS.captureScreenshot not available, using fallback');
        return this._getFallbackScreenshot(tabId, size, width, height);
      }
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
  ...
  /**
   * Fallback implementation for captureScreenshot when chrome.browserOS is not available
   */
  private async _getFallbackScreenshot(
    tabId: number,
    size?: ScreenshotSizeKey,
    width?: number,
    height?: number,
    ...
    try {
      // Use standard Chrome tabs API for screenshots
      ...
      if (!dataUrl) {
        throw new Error('Failed to capture screenshot using fallback method');
      }

      // If no specific size requested, return as-is
      if (!size && !width && !height) {
        console.log(`[BrowserOSAdapter] Fallback screenshot captured for tab ${tabId}`);
        return dataUrl;
      }

      // If size or dimensions specified, we'd need to resize the image
      // For now, just return the full screenshot with a warning
      console.warn(`[BrowserOSAdapter] Fallback screenshot: size/dimension options not supported, returning full screenshot`);
      return dataUrl;
    } catch (error) {
      console.error('[BrowserOSAdapter] Fallback screenshot failed:', error);
      // Return a simple data URL for a minimal image
