  ...
  return new DynamicStructuredTool({
    ...
    func: async (args: MoondreamVisualTypeInput) => {
      try {
        ...
        if (!apiKey) {
          context.getPubSub().publishMessage(
            PubSubChannel.createMessage("⚠️ Vision API not configured, falling back to text-based interaction", "error")
          );
          return JSON.stringify({
            ok: false,
            error: "Vision API key not provided. Please configure MOONDREAM_API_KEY in .env file to use visual tools.",
            suggestion: "Try using text-based tools or configure the API key"
          });
        }
