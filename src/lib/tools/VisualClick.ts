  ...
  return new DynamicStructuredTool({
    ...
    func: async (args: MoondreamVisualClickInput) => {
      try {
        ...
        if (!apiKey) {
          context.getPubSub().publishMessage(
            PubSubChannel.createMessage("⚠️ Vision API not configured, trying alternative approach", "error")
          );
          return JSON.stringify({
            ok: false,
            error: "Vision API key not provided. Please configure MOONDREAM_API_KEY in .env file to use visual tools.",
            suggestion: "Try using element-based clicking or configure the API key"
          });
        }
