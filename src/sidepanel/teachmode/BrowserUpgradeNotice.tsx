...
export function BrowserUpgradeNotice({ currentVersion, onDismiss, className }: BrowserUpgradeNoticeProps) {
  const handleUpgradeClick = () => {
    // Neutralized external BrowserOS link - open disabled
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-foreground">
          PAgentic Browser Update Required
        </p>
        <p className="mt-0.5 text-xs text-muted-foreground">
          You're using PAgentic Browser {currentVersion || 'an older version'}. Please upgrade to use Teach mode.
        </p>
      </div>
