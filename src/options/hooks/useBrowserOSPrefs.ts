...
const DEFAULT_PAGENTIC_PROVIDER: LLMProvider = {
  id: 'pagentic',
...
export function useBrowserOSPrefs() {
  const [providers, setProviders] = useState<LLMProvider[]>([DEFAULT_PAGENTIC_PROVIDER])
  const [defaultProvider, setDefaultProviderState] = useState<string>('pagentic')
          ...
          if (msg.type === MessageType.WORKFLOW_STATUS) {
            ...
            if (payload?.data?.providersConfig) {
              ...
              const migratedProviders = config.providers.map(p => ({
                ...p,
                isDefault: p.isDefault !== undefined ? p.isDefault : (p.id === 'pagentic' || p.id === 'browseros')
              }))
              setProviders(migratedProviders)
              setDefaultProviderState(config.defaultProviderId || 'pagentic')
              setIsLoading(false)
            }
          }
        }
  ...
  const deleteProvider = useCallback(async (providerId: string) => {
    ...
    if (providerId === defaultProvider) {
      const pagenticProvider = remainingProviders.find(p => p.id === 'pagentic' || p.id === 'browseros')
      nextDefaultId = pagenticProvider?.id || remainingProviders[0]?.id || 'pagentic'
